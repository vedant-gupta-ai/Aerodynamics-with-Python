...
def compute_lift(flap_rotn):

    x_fn, y_fn = rotate(1.03, -0.054, flap_rotn, x_f, y_f)

    panels_af = numpy.empty(N, dtype=object)
    for i in range(N):
    ...
    freestream = Freestream(alpha=4.0, u_inf=1.0)
...
class Freestream:
    def __init__(self, alpha = 0.0, u_inf = 1.0):
        self.u_inf = u_inf
...
while True:
    flap_a = -4.0
    flap_b = -5.0
    flap_angle = (flap_a + flap_b) / 2

    if compute_lift(flap_angle) == 0:
        flap_lift_zero = flap_angle
        print(f"Zero lift AoA = {flap_lift_zero}")
        break
    elif compute_lift(flap_a) * compute_lift(flap_angle) < 0:
        flap_b = flap_angle
    elif compute_lift(flap_b) * compute_lift(flap_angle) < 0:
        flap_a = flap_angle
